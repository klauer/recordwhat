...
class RecordBase(Device):
    ...
    forward_link = FieldCpt(EpicsSignal, '.FLNK$', string=True)
    lock_count = FieldCpt(EpicsSignalRO, '.LCNT')
    record_name = FieldCpt(EpicsSignalRO, '.NAME$', string=True)
    ...
    scan_disable_input_link = FieldCpt(EpicsSignal, '.SDIS$', string=True)
    ...
    time_stamp_event_link = FieldCpt(EpicsSignal, '.TSEL$', string=True)
